...
{
	char szSrc[4096] = {0};
	memcpy(szSrc, szInput, nLen);
	unsigned char szKeyChar[8] = {0};
	memcpy(szKeyChar, szKey, nKeyLen < 8 ? nKeyLen : 8);
	DES des;
	int nBlocks = ((nLen % 8)== 0) ? (nLen / 8) : (nLen / 8 + 1);
	int nRet = des.encrypt(szKeyChar, (unsigned char *)szSrc, nBlocks);
	if(nRet)
	{
		memcpy(szOut, szSrc, nBlocks * 8);
		nRet = nBlocks * 8;
	}
	return nRet;
}
...
int Decrypt(char *szOut, const char *szInput, int nLen, const char *szKey, int nKeyLen)
{
	...
	DES des;
	int nBlocks = nLen / 8;
	int nRet = des.decrypt(szKeyChar, (unsigned char *)szSrc, nBlocks);
	if(nRet)
	{
		memcpy(szOut, szSrc, nBlocks * 8);
	}
